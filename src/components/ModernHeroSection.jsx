...
export default function ModernHeroSection() {
  ...
  const formattedDate =
    event.formattedDate?.[isRtl ? "ar" : "en"] ||
    event.formattedDate ||
    (isRtl ? "22 أغسطس 2026" : "22 Aug 2026");
    ...
    <section className="relative w-full h-[100dvh] min-h-[100dvh] flex flex-col justify-between items-center text-center px-4 sm:px-8 pt-16 sm:pt-20 pb-6 sm:pb-8 overflow-hidden select-none">
      ...
      <div className="absolute inset-0 w-full h-full min-w-full min-h-full overflow-hidden z-0 pointer-events-none">
        <motion.img
          ...
          className="absolute inset-0 w-full h-full !w-full !h-full min-w-full min-h-full object-cover object-[48%_center] sm:object-center will-change-transform brightness-[0.92]"
          loading="eager"
          fetchPriority="high"
        />

        {/* Ambient Vignette Shadows & Dark Filter */}
        <div className="absolute top-0 inset-x-0 h-44 bg-gradient-to-b from-black/65 via-black/25 to-transparent pointer-events-none" />
        <div className="absolute bottom-0 inset-x-0 h-72 bg-gradient-to-t from-black/85 via-black/45 to-transparent pointer-events-none" />
      ...
      <div className="relative z-10 flex flex-col items-center justify-center my-auto px-2 w-full max-w-5xl">
        ...
        {/* Poetic Invitation Subtitle ("يتشرفان...") */}
        <motion.p
          initial={{ opacity: 0, y: 15 }}
          animate={isReady ? { opacity: 1, y: 0 } : { opacity: 0, y: 15 }}
        ...
        <motion.div
          ...
          className="mt-4 inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/15 backdrop-blur-md border border-white/25 text-[#f8f7f3] text-[11px] xs:text-xs font-century shadow-sm"
        >
